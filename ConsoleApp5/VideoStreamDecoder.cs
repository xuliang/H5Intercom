    ...
    public sealed unsafe class VideoStreamDecoder : IDisposable
    {
        ...
        public VideoStreamDecoder(string url)
        {
            ...
            for (var i = 0; i < _pFormatContext->nb_streams; i++)
                ...
                else if (_pFormatContext->streams[i]->codecpar->codec_type == AVMediaType.AVMEDIA_TYPE_AUDIO)
                {//有可能有多路音轨
                    Console.WriteLine("audioStream = {0}", i);
                    AVDictionaryEntry* t = null;
                    while ((t = ffmpeg.av_dict_get(_pFormatContext->streams[i]->metadata, "", t, ffmpeg.AV_DICT_IGNORE_SUFFIX)) != null)
                    {
                        var key = Marshal.PtrToStringAnsi((IntPtr)t->key);
                        var value = Marshal.PtrToStringAnsi((IntPtr)t->value);
