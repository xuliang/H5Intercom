...
namespace ConsoleApp11
{
    public unsafe class RTMPTask
    {
        ...
        {
            bos.Flush();
            networkStream.Write(bos.ToArray());
            networkStream.Flush();
        //bos.reset();
    }
    public void Write(byte[] buff)
        ...
        int read_buffer(void* opaque, byte* buf, int buf_size)
        {
            Console.WriteLine("-------------------");
            AVIOBufferContext* op = (AVIOBufferContext*)opaque;
            int len = buf_size;
            if (op->pos + buf_size > op->totalSize)
            {
                len = op->totalSize - op->pos;
            }
            Marshal.Copy(buff,  op->pos, new IntPtr(buf), len);
        ...
        int _ReadNetPacket(byte* buf, int buf_size)
        {
            ...
            while ((block = networkStream.Read(buffer, read, buffer.Length - read)) > 0)
            {
                // 重新设定读取位置
                read += block;
        ...
        void rtmp()
        {
            ...
            /*open input file*/
            avio_in = ffmpeg.avio_alloc_context(inbuffer, 32768, 0, &g_avbuffer_in, new avio_alloc_context_read_packet(ReadNetPacket), null, null);
            if (avio_in == null)
                return;
